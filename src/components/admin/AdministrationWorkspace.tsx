...
type AdministrationScope = 'system' | 'PRZEMIALY' | 'CZESCI';

export default function AdministrationWorkspace({ scope }: { scope: AdministrationScope }) {
  const currentUser = useUiStore((state) => state.user);
  const allowed = scope === 'system'
    ? isHeadAdmin(currentUser)
    : isWarehouseAdmin(currentUser, scope);
  if (!allowed) return <Card><p className="text-sm text-muted">Brak dostepu.</p></Card>;
  return <AdministrationContent key={scope} scope={scope} />;
}

function AdministrationContent({ scope }: { scope: AdministrationScope }) {
  const searchParams = useSearchParams();
  const currentUser = useUiStore((state) => state.user);
  const activeWarehouse = scope === 'system' ? null : scope;
  const managesPrzemialy = scope === 'PRZEMIALY';
  const managesCzesci = scope === 'CZESCI';
  const toast = useToastStore((state) => state.push);
  const queryClient = useQueryClient();
  const today = getTodayKey();
  const canManageAccounts = scope === 'system' && isHeadAdmin(currentUser);
  const glowClass = 'ring-2 ring-[rgba(255,122,26,0.45)] shadow-[0_0_0_3px_rgba(255,122,26,0.18)]';
  const { data: auditData } = useQuery({ queryKey: ['audit'], queryFn: getAudit, enabled: managesPrzemialy });
  const { data: warehousesData } = useQuery({
    queryKey: ['warehouses-admin'],
    queryFn: () => getWarehousesAdmin(),
    enabled: managesPrzemialy
  });
  ...
  useEffect(() => {
    if (!managesPrzemialy || typeof window === 'undefined') return;
  ...
  useEffect(() => {
    if (!managesPrzemialy || typeof window === 'undefined' || !tabReady) return;
  ...
  }, [managesPrzemialy, przemialyTab, tabReady]);
  ...
  const { data: inventoryDetail = [] } = useQuery({
    ...
    enabled: managesPrzemialy && Boolean(inventoryForm.warehouseId && inventoryForm.locationId)
  });
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={scope === 'system' ? 'Panel administratora' : 'Zarządzanie modułem'}
